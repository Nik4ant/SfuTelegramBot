...
def empty(telegram_id: int) -> UserModel | None:
	"""@return: Returns None - if error occurred"""
	try:
...
def from_sfu_login(telegram_id: int, sfu_login: str) -> UserModel | None:
	"""@return: Returns None - if error occurred"""
	try:
		_now = datetime.now(SFU_UNI_TIMEZONE)
		cur.execute(
...
def add_group_to(telegram_id: int, group_name: str) -> None:
	cur.execute(
...
def add_subgroup_to(telegram_id: int, subgroup: str) -> None:
	cur.execute(
...
def clear_by_id(telegram_id: int, _sync_time_now: datetime = None) -> bool:
	"""
...
def remove_by_id(telegram_id: int) -> None:
	try:
